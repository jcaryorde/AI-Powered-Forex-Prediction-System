...
int OnInit()
{
   ...
   for(int i=0; i<total; i++)
   {
      ...
      handle_atr[i] = iATR(symbols[i], _Period, ATR_Period);
      handle_bb[i]  = iBands(symbols[i], _Period, BB_Period, 0, BB_Deviation, PRICE_CLOSE);
   ...
   Print("Volatility Extreme EA initialized. Monitoring 5 assets.");
...
bool IsVolumeClimax(string symbol)
{
   ...
   if(CopyTickVolume(symbol, _Period, 0, 21, volume) < 21) return false;
